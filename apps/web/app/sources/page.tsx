...
export default async function SourcesPage() {
  ...
  return <main className="page"><div className="container">{viewer.fixture && <aside className="fixture-banner" role="status"><strong>開發展示資料</strong><span>目前數量不代表全臺即時或完整覆蓋；只有「正式同步」才通過真實資料庫驗收。</span></aside>}<section className="hero"><div><div className="eyebrow">來源監測</div><h1>資料來源健康狀態</h1><p>把「介接進度」和「最近同步狀態」分開看。未實作只代表列入藍圖，試運轉也不等於已正式上線。</p></div><div className="hero-stat"><strong>{activeCount}</strong><span className="muted">個已通過真實資料庫同步的正式來源</span></div></section>
