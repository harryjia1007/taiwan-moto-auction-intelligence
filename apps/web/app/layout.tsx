...
import { Database, Gauge, Heart, LogOut, Search } from "lucide-react";
import { getViewer } from "@/lib/auth";
import "./globals.css";

export const metadata: Metadata = { title: "臺灣汽機車拍賣情報", description: "可信、可追溯的臺灣政府汽機車拍賣情報", robots: { index: false, follow: false } };
...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang="zh-TW"><body>
    {viewer && <header className="app-header"><div className="container header-inner">
      <Link href="/motorcycles?view=active" className="brand" aria-label="臺灣汽機車拍賣情報首頁">
        <span className="brand-mark"><Gauge size={19} /></span>
        <span className="brand-copy"><small>臺灣官方標售情報</small><strong>汽機車拍賣情報</strong></span>
      </Link>
