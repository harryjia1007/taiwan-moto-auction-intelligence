...
export const metadata: Metadata = {
  title: "臺灣汽機車拍賣情報｜互動作品展示",
  description: "使用完全合成資料展示汽機車拍賣情報的篩選、風險判斷與證據導向設計。",
...
export default async function DemoPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="demo-page">
    <header className="demo-header"><div className="container demo-header-inner"><Link href="/demo" className="demo-brand">HARRY JIA <span>專案</span></Link><nav><Link href="#architecture">專案架構</Link><Link href="/legal/data-use">資料使用</Link><Link href="/login"><LockKeyhole size={15}/> 私人情報台</Link></nav></div></header>
    <div className="container">
      ...
      <section className="demo-hero"><div><p className="eyebrow">臺灣汽機車拍賣情報</p><h1>先看能不能買，<br/>再決定值不值得追。</h1><p>把分散的汽機車拍賣條件整理成可搜尋、可比較、可追溯的決策介面。正式情報與原始證據維持私人存取。</p><div className="demo-hero-actions"><a href="#market" className="button">體驗找車流程 <ChevronRight size={16}/></a><Link href="/legal/disclaimer">閱讀使用限制</Link></div></div><div className="demo-architecture-card"><Database size={24}/><strong>以官方證據為核心</strong><span>官方來源 → 私有保存 → 保守正規化 → 欄位證據 → 私人儀表板</span><small>公開作品層只使用獨立合成資料</small></div></section>
      <section className="demo-market" id="market"><div className="demo-section-heading"><div><span>互動展示</span><h2>用真正找車的方式操作</h2></div><p>先選案件狀態，再用法定級別與排氣量縮小範圍。</p></div>
      ...
      <section className="demo-explainer" id="architecture"><div><span>01</span><h2>資料不亂猜</h2><p>未知、否定、衝突與無法測試分開保存；排氣量不會被拿來冒充法定級別。</p></div><div><span>02</span><h2>證據可回查</h2><p>私人正式站保留原始快照、雜湊校驗值與欄位證據，但公開作品層不散布附件。</p></div><div><span>03</span><h2>來源會停手</h2><p>遇到網站禁止自動存取、驗證碼、登入或授權不明時，系統預設停止，不以涵蓋率犧牲合規。</p></div></section>
      <footer className="demo-footer"><div><strong>臺灣汽機車拍賣情報</strong><span>Harry Jia 個人作品・非政府官方服務</span></div><nav><Link href="/legal/privacy">隱私</Link><Link href="/legal/data-use">資料使用</Link><Link href="/legal/disclaimer">免責與更正</Link><a href="https://data.gov.tw/license" target="_blank" rel="noreferrer">政府資料開放授權 <ExternalLink size={12}/></a></nav></footer>
    </div>
  </main>;
}
