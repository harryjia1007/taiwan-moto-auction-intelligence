...
Scheduled public ingestion requires encrypted `SUPABASE_URL` and `SUPABASE_SECRET_KEY` repository secrets; the legacy `SUPABASE_SERVICE_ROLE_KEY` remains a fallback during migration. The workflow validates both before making a source request, then runs the Taipei Shwoo and MOJ centralized-auction publishers twice daily at 09:30 and 21:30 Asia/Taipei. Each publisher preserves raw artifacts privately before updating the public projection. Judicial discovery is disabled because the current robots policy disallows automated paths; Administrative Enforcement remains human-assisted because of CAPTCHA; Government e-Procurement remains paused for authorization review. The workflow never logs into or bids on a source site.
