...
For a targeted diagnostic run, open the `Scheduled official-source ingestion` GitHub Actions workflow, choose **Run workflow**, and set `source` to one of `moj_auction`, `pcc`, `customs`, or `moj_enforcement_cms`. The default `all` retains the existing manual full run; the twice-daily scheduled event always checks all four. A nonselected matrix job skips checkout, runtime setup, installation, credential validation, and publication, so its green/skipped job entry is **not** evidence that the source synchronized. Inspect the selected job's conclusion and its matching database run before reporting success.
