...
公開頁是精簡的正式案件查詢工具，從 Supabase 唯讀公開 view 取得經整理的案件資料，提供進行中、30 天內、已結束、我的收藏、關鍵字、法定級別、排氣量及照片篩選。排氣量級距為 125 以下、126–150、151–250、251–550、550 以上及未提供，並顯示各級距筆數。每筆案件顯示官方已提供的車牌、案號、價格、日期、機關、地點、資格、領牌狀態與車況；有官方照片才顯示媒體區。公開案件詳細頁可連回官方公告，並可同時比較 2–3 台。

## 公開邊界

- 公開：正規化案件欄位、官方公告連結、允許公開載入的官方照片。
- 不公開：Supabase secret／service-role key、原始 artifacts、私人 Storage、完整證據庫、伺服器端收藏、來源維運資料及 owner 帳號。公開版收藏、比較與造訪時提醒只存在使用者瀏覽器，不會寫入正式資料庫。
