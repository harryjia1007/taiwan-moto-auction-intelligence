...
The anonymous `public_live_motorcycle_listings` table is a separate least-privilege projection. It masks the final two or three characters of a parsed plate only while its verified auction end time is within the retention window; otherwise plate text is suppressed and the plate field stays null. It excludes engine, frame and VIN identifiers entirely. Taiwan IDs, phone/email values and role-labelled names are redacted from every projected text field. Official text labelled `車牌`, `牌照號碼`, or `車號` is also checked for numeric-hyphenated and compact alphanumeric plates the source parser may have missed; those unparsed values are suppressed, not promoted to normalized plate data. Plausible hyphenated and compact plate tokens in official URLs or source record IDs cause an origin fallback or hashed ID; affected document URLs are removed instead of rewritten into broken links. Full identifiers and unredacted source evidence remain only in owner-only normalized records, evidence and private artifacts.
